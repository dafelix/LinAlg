...
\isubsection{Integritätsbereich / Integritätsring}
...
\isubsection{Algebraische Abgeschlossenheit}
\begin{definition}
...
\end{center}
Ein Körper K ist \textbf{algebraisch abgeschlossen}\index{Algebraische Abgeschlossenheit}, falls jedes f $\in$ K[x] mit Grad(f) $\ge$ 1 in K eine Nullstelle hat (also über K zerfällt).
\end{definition}
...
\isection{Lotraum}
\begin{theorem}
Sei $M \subseteq V$ eine Menge. $M^\perp = \{v\in V | (v,m)=0~\forall m \in M\}$ definiert den \textbf{Lotraum} von M. $M^\perp \subseteq V$ ist ein Unterraum.
