...
\section{Ringe}
\begin{enumerate}
	\item R ist ein euklidischer Ring, falls es eine funktion $\phi : R\backslash \{0\} \rightarrow \mathbb{N}_0$ gibt, sodass gilt:
...
Die Elemente $\pi_1^{e_1,1},...,\pi_1^{e_s,1},\pi_2^{e_1,2},...,\pi_2^{e_s,2},...,\pi_t^{e_1,t},...,\pi_t^{e_s,t}$ mit 
\begin{align*}
\begin{array}{ll}
...
\end{pmatrix}
\end{align*}
Bemerkung: Aus der JNF lässt sich das Minimalpolynom $\mu_A$ direkt ablesen. Sortiere Jordankästchen nach EW und nach Größe der Jordankästchen.
\begin{align*}
\begin{pmatrix}
J(\alpha_1,e_{1,1}) & & & & & & 0\\
& \ddots & & & & & \\
...
\end{enumerate}
Bemerkung: V zusammen mit einem inneren Produkt nennt man einen \textbf{euklidischen Raum} (falls $K=\mathbb{R}$) bzw. \textbf{unitären Raum} (falls $K=\mathbb{C}$).
