...
\begin{compactenum}
\item Eine Abbildung f : V $\to$ W heißt linear (oder Homomorphismus), falls für alle $a_1, a_2, a \in V$ und $\alpha \in K$ gilt:
...
\begin{itemize}
\item Lemma 5.8: Sei V ein K-Vektorraum und seien U $\subseteq$ W $\subseteq$ V linearere Unterräume. Dann gilt:
\begin{compactenum}
...
\begin{compactenum}
\item Für $f, g \in Hom_K(V, W), \alpha \in K$ und $a \in V$ setze\\
...
\begin{compactenum}
\item[(1)] Ein \textbf{Homomorphismus} (oder Gruppenhomomorphismus) ist eine Abbildung f: G $\to$ H, die mit den Gruppenstrukturen verträglich ist, d.h. für $g_1, g_2 \in G$ gilt:\\
...
\item Bemerkungen:
\begin{compactenum}
\item Der Fundamentalsatz der Algebra besagt, dass jedes f $\in$ $\mathbb{C}$[x] mit Grad(f) $\ge$ 1 in $\mathbb{C}$ einen Nullstelle besitzt. Also ist $\mathbb{C}$ algebraisch abgeschlossen. Insbesondere gilt: Sind K $\subseteq$ $\mathbb{C}$ Körper und ist f $\in$ K[x], so liegen alle Nullstellen von f in $\mathbb{C}$.
...
\end{center}
\item Ist dimK V = n < $\infty$, so ist f diagonalisierbar (über K), falls es Eigenwerte $\alpha_1, …, \alpha_r \in \sigma(f)$ gibt, so dass gilt $V = \oplus_{i=1}^{r} V_f (\alpha_i)$.\\
