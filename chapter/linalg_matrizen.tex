...
\section{Diagonalisierbarkeit\index{Diagonalisierbarkeit}}
...
Sei R ein komm. Ring.
\begin{enumerate}
	\item Seien $A,B \in K^{n \times m}$. Dann heißen $A$ und $B$ äquivalent\index{äquivalente Matrizen}, falls es $P \in Gl_n(R), Q \in Gl_m(R)$ gibt mit $B=P^{-1}AQ$. In Zeichen $A \sim B$.
	\item Seien $A,B \in M_n(R) = K^{n \times n}$. Dann heißen $A$ und $B$ ähnlich\index{ähnliche Matrizen} oder konjugiert, falls es $S \in Gl_n(R)$ gibt mit $B=S^{-1}AS$. In Zeichen $A \approx B$.
\end{enumerate}
\end{definition}

\section{positiv definit\index{positiv definit}}
\begin{definition}
...
\section{Gram-Schmidt-Verfahren\index{Gram-Schmidt-Verfahren}}
...
\begin{definition}
Eine Matrix $A \in M_n(K)$ heißt unitär\index{unitär}, falls $AA^* = E$\\
Falls $K = \mathbb{R}$ heißt $A$ orthogonal\index{orthogonal} und $A^{-1} = A^t$
\end{definition}
\begin{theorem}
...
Dann heißt B = ($\beta(v_i, w_j))_{ij}$ $\in K^{n \times m}$ die Strukturmatrix\index{Strukturmatrix} von $\beta$ bez. der gewählten Basen.
\end{definition}
