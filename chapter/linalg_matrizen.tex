...
\isection{Normalformen}
\isubsection{Frobeniussche Normalform}
\begin{theorem}
...
\end{theorem}
\subsection{Weierstraß'sche Normalform}\index{Weierstraß. Normalform}
\begin{theorem}
...
\end{pmatrix}
\end{align*}
Die Polynome $h_1,...,h_m$ sind genau die \textbf{Invariantenteiler}\index{Invariantenteiler} von $M_A(x)$
\end{theorem}

\isubsection{Jordan Normalform}
\begin{theorem}
Sei $A \in M_n(K)$ und $\chi_A$ zerfalle vollständig in Linearfaktoren. Dann gibt es bis auf Reihenfolge ein eindeutig bestimmtes System von \textbf{Jordanmatrizen}\index{Jordanmatrix} $J_1,...,J_m$:
\begin{align*}
A \simeq 
\begin{pmatrix}
J_1 & & 0\\
...
\begin{definition}
Eine Matrix $A \in M_n(\mathbb{C})$ heißt normal\index{Normale Matrix}, falls $AA^* = A^*A$.
\end{definition}
\begin{lemma}\index{Diagonalisierbarkeit}
...
Sei R ein komm. Ring.
\begin{enumerate}
	\item Seien $A,B \in K^{n \times m}$. Dann heißen $A$ und $B$ \textbf{äquivalent}\index{Aquivalente Matrizen}, falls es $P \in Gl_n(R), Q \in Gl_m(R)$ gibt mit $B=P^{-1}AQ$. In Zeichen $A \sim B$.
	\item Seien $A,B \in M_n(R) = K^{n \times n}$. Dann heißen $A$ und $B$ \textbf{ähnlich}\index{Ähnliche Matrizen} oder konjugiert, falls es $S \in Gl_n(R)$ gibt mit $B=S^{-1}AS$. In Zeichen $A \approx B$.
\end{enumerate}
\end{definition}

\isection{Positiv definite Matrizen}
\begin{definition}
...
\isection{Gram-Schmidt-Verfahren}
...
\begin{definition}
Eine Matrix $A \in M_n(K)$ heißt \textbf{unitär}\index{Unitäre Matrix}, falls $AA^* = E$\\
Falls $K = \mathbb{R}$ heißt $A$ \textbf{orthogonal}\index{Orthogonale Matrix} und $A^{-1} = A^t$
\end{definition}
\begin{theorem}
...
\begin{definition}
Seien $A, B \in M_n(K)$.\\
$A$ ist \textbf{unitär ähnlich}\index{Unitär ähnliche Matrizen} zu $B$, falls es eine unitäre Matrix $U$ gibt mit $B = U^*AU$\\
$A$ ist \textbf{orthogonal ähnlich}\index{Orth. ähnliche Matrizen} zu $B$, falls es eine orthogonale Matrix $O$ gibt mit $B = O^tAO$
\end{definition}
